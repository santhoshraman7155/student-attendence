...
def admin_dashboard(request):
    today = localtime().date()  # Get today's date

    total_students = Student.objects.count()
    total_attendance = Attendance.objects.filter(date=today).count()  # Count only today's attendance
...
def student_attendance_list(request):
    ...
    # Get today's date or use provided filter
    today = timezone.now().date()
    try:
        date_filter = datetime.strptime(date_filter, "%Y-%m-%d").date() if date_filter else today
    except ValueError:
        date_filter = today  # Default to today if invalid

    # Build attendance filters
    attendance_filter = Q(date=date_filter)
    if status_filter:
        attendance_filter &= Q(status__iexact=status_filter)
    if search_query:
        attendance_filter &= Q(student__name__icontains=search_query)
    if roll_no_filter:
        attendance_filter &= Q(student__roll_no__icontains=roll_no_filter)

    # Find the first recorded attendance per student per day (using Min ID)
    unique_attendance_ids = (
        Attendance.objects.filter(attendance_filter)
        .values('student', 'date')  # Group by student & date
        .annotate(min_id=Min('id'))  # Get the first (minimum ID) record per student per day
        .values_list('min_id', flat=True)  # Extract IDs of unique records
    )

    # Fetch the unique attendance records using the IDs found
    attendance_records = Attendance.objects.filter(id__in=unique_attendance_ids).select_related('student')

    # Organize attendance data per student
    ...
    for record in attendance_records:
        ...
        student_attendance_data[student_id]['attendance_records'].append(record)
        student_attendance_data[student_id]['attendance_count'] += 1

    # Optimized counting
    total_attendance_count = attendance_records.count()
    total_present = attendance_records.filter(status__iexact="present").count()
    total_absent = attendance_records.filter(status__iexact="absent").count()

    context = {
        'student_attendance_data': student_attendance_data.values(),
